---
name: wrapup
description: "用户要求收尾或复盘任务时，归档结果、经验和待办。"
---
...
先加载 `github-project-governance` skill；所有项目读写、配额检查和写后回读必须由它执行。禁止使用 GitHub CLI 的 Project 子命令，也禁止在本 skill 内自写项目 GraphQL。
...
2. 按治理 skill 回读当前状态；已经是 `Done` / `完成` 就幂等跳过。否则读取 Status 字段与真实 option ID，再推进完成态：#9 使用 `完成`，#5 使用 `完成`。未来专项只接受 `Done` / `完成` 的唯一精确匹配，找不到或不唯一就询问，禁止猜测。
...
> task-ledger 账本已废弃（2026-08-06），不再调 task\_\* 工具。
