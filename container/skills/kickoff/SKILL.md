...
判灯在 B5 汇报**前**完成（模板「下一步」行填的就是分流结果）；汇报发出后**不等回复**：全绿/黄直接进入实现；含红灯项按挂账规则处理（红灯及其依赖挂账，独立部分继续）。进入实现时，先核验 Step 3.5.1 的项目路由和 Step 3.5.2 Gate，缺失就回去补齐；再执行 Step 3.5.3 推进到开工态。驾驶舱追加一条"灯色分流通过（红灯挂账 N 项），进入实现"，然后才开始写代码 + 单测。
