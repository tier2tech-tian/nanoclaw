---
name: kickoff
description: "启动尚未明确范围的开发任务，形成需求与验收标准。"
---
...
实现和审查时按当前改动模块核对Wiki契约；已读且仍在上下文中的材料无需重复加载。交付约定见 [mp-implement](../mp-implement/references/delivery.md)。
...
判灯在 B5 汇报前完成；全绿/黄直接进入实现，红灯及其依赖挂账，独立部分继续。先核验 Step 3.5.1 的项目路由与 Step 3.5.2 的跟踪绑定，再按 Step 3.5.3 推进开工态并记录驾驶舱；实现与交付统一执行 [mp-implement](../mp-implement/SKILL.md)，不在此重复阶段或测试顺序。
