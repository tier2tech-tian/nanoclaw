...
判灯在 A2 汇报**前**完成（模板「下一步」行填的就是分流结果）；汇报发出后**不等回复**，按灯色执行。对修复方案中的每个执行动作按「自主分级」章节判灯色：
...
判灯在 B5 汇报**前**完成（模板「下一步」行填的就是分流结果）；汇报发出后**不等回复**：全绿/黄直接进入实现；含红灯项按挂账规则处理（红灯及其依赖挂账，独立部分继续）。进入实现时，先核验 Step 3.5.1 的项目路由和 Step 3.5.2 Gate，缺失就回去补齐；再执行 Step 3.5.3 推进到开工态。驾驶舱追加一条"灯色分流通过（红灯挂账 N 项），进入实现"，然后**按 `implement` skill（`container/skills/implement/SKILL.md`）从 Step 0 起执行**写代码 + 单测 + 双轨审查 + 提 PR 的完整流程——kickoff 到此交棒，实现步骤不在本文件里。
