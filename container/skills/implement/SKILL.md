...
> 无 Agent 工具的运行时（Codex 等）：按 kickoff「自主分级」章节的评审 fallback——用下方审查标准逐项自查并输出结构化结论，不得跳过。
...
> 完整 E2E 执行+汇报流程见 `e2e-testing` skill。本步骤只管用例设计和执行。
...
**Gate**: 汇报含三段式（设计思路 + 用例列表 + 逐条结果）+ 非挂账用例全部通过（含红灯的用例已挂账列明）
...
**E2E**（如有）：
- 设计思路：{覆盖策略 + 回归范围，一两句}
- {N} 个用例全部通过（用例列表 + 逐条结果附后；挂账用例单独列明）

**需要你决定**（如有挂账，必列；无则省略此段）：
- {红灯项：背景 + 选项 + 我的建议，含随挂的依赖项}
